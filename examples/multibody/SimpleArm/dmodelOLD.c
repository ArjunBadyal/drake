    ...
    while (!terminate__) {
                          ...
                          , end_stm_ref0);
      {
        ...
        while (!outputDone) {
          ...
          if (_output_.type == stm_ref0_output_Return) {
            {
          ...
          else if (_output_.type == stm_ref0_output_PrePick) {
            {
              int status = pthread_mutex_lock(&end_ctrl_ref0->mutex);
              if (status != 0)
                err_abort(status, "Lock mutex");
    ...
    while (!terminate__) {
                         ...
                         , end_ctrl_ref0);
      {
        ...
        while (!outputDone) {
          ...
          if (_output_.type == C_ctrl_ref0_output_Return) {
            {
              int status = pthread_mutex_lock(&end_PickPlace->mutex);
              if (status != 0)
                err_abort(status, "Lock mutex");
