#include "drake/examples/multibody/SimpleArm/PthreadLeafSystem"
...
//template <typename T>
PthreadLeafSystem::PthreadLeafSystem(): LeafSystem<double>() {
  // Declare depth input port.
  PthreadLeafSystem_input_port_ =
...
void SimpleArm::PthreadLeafSystem::CalcOutput(const Context<double>& context, BasicVector<double>* output) const{
  ...
  const auto &depth = PthreadLeafSystem_input_port().Eval<drake::geometry::QueryObject<double>>(context);
  auto val = depth.inspector().GetAllGeometryIds();
